...
```{r, eval=FALSE}
# linear model of post treatment data
m1_ols <- lm(
    waste_piles ~ treat,
...
assumption = variance is proportional to the mean - "dispersion"
```{r, eval=FALSE}
m2_poisson <- glm(
    waste_piles ~ treat,
...
# Change in percent (if its over 1, positive change - if its under 1 , its a reduction)
exp(-0.38) - 1
# This is estimating a 31% reduction in waste piles
```
